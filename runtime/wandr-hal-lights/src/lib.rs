//! wandr-hal-lights — shared `android.hardware.light` ILights access.
//!
...
mod binder_path {
    ...
    /// (same as wandr-hal-display/sensors — idempotent per process).
    fn ensure_process_state() {
        static INIT: OnceLock<()> = OnceLock::new();
        INIT.get_or_init(|| {
            if !std::path::Path::new("/dev/binder").exists() {
                log::warn!("wandr-hal-lights: /dev/binder absent — lights HAL unavailable");
                return;
            }
    ...
    pub fn set_backlight(frac: f32, sensor: bool) -> bool {
        ...
        let lights = match svc.r#getLights() {
            Ok(v) => v,
            Err(e) => {
                log::debug!("wandr-hal-lights: getLights failed: {e:?}");
                return false;
            }
        };
        ...
        for hw in lights.iter().filter(|h| h.r#type == LightType(LIGHT_TYPE_BACKLIGHT)) {
            match svc.r#setLightState(hw.r#id, &state) {
                Ok(()) => any = true,
                Err(e) => log::debug!("wandr-hal-lights: setLightState(id={}) failed: {e:?}", hw.r#id),
            }
        }
        any
    }
