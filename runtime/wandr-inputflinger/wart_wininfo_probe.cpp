// wandr_wininfo_probe — minimal diagnostic: register a SurfaceFlinger
...
// problem. Run: wandr_wininfo_probe (as root or via wandr-launch).

#define LOG_TAG "wandr_wininfo_probe"
#include <log/log.h>
...
public:
    void onWindowInfosChanged(const gui::WindowInfosUpdate& u) override {
        ALOGE("WANDR-PROBE onWindowInfosChanged: %zu windows, %zu displays",
              u.windowInfos.size(), u.displayInfos.size());
        for (const auto& w : u.windowInfos) {
            ALOGE("WANDR-PROBE   win '%s' frame=[%d,%d,%d,%d] cfg=0x%x",
...
int main() {
    ...
    ALOGE("WANDR-PROBE addWindowInfosListener -> %d, initial windows=%zu displays=%zu",
