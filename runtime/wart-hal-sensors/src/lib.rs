//! wart-hal-sensors — shared sensor HAL access (task 77).
//!
...
//! used by both wart-host (guest-facing `sensors` WIT) and the arbiter's
//! sensor-driver thread. Exposes **neutral** structs ([`HalSensor`],
//! [`HalSample`]) — each consumer maps them to its own vocabulary (WIT `Kind`,
//! arbiter `SensorKind`).
//!
//! Mechanism (lifted from the original `wart-host/src/sensors_impl.rs`): a `Bn`
...
mod binder_path {
    ...
    /// the init; in wart-host (which inits `ProcessState` itself) the second
    ...
    fn ensure_process_state() {
        ...
        INIT.get_or_init(|| {
            if !std::path::Path::new("/dev/binder").exists() {
                log::warn!("wart-hal-sensors: /dev/binder absent — sensors unavailable");
                return;
            }
