...
//! these calls must therefore run as uid 1000 (e.g. via `wart-launch` or
...
fn ensure_process_state() {
    ...
    INIT.get_or_init(|| {
        if !std::path::Path::new("/dev/binder").exists() {
            log::warn!("wart-hal-net: /dev/binder absent — dnsresolver unavailable");
            return;
        }
...
fn service() -> Option<rsbinder::Strong<dyn IDnsResolver>> {
    ...
    match rsbinder::hub::get_interface::<dyn IDnsResolver>("dnsresolver") {
        Ok(svc) => Some(svc),
        Err(e) => {
            log::warn!("wart-hal-net: get dnsresolver failed: {e:?}");
            None
        }
    }
}
...
fn netd_service() -> Option<rsbinder::Strong<dyn INetd>> {
    ...
    match rsbinder::hub::get_interface::<dyn INetd>("netd") {
        Ok(svc) => Some(svc),
        Err(e) => {
            log::warn!("wart-hal-net: get netd failed: {e:?}");
            None
        }
    }
}
