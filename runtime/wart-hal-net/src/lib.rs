//! wart-hal-net — ART-off WiFi/IP bring-up primitives (task 88, M1).
//!
...
/// The netId of the single wart-managed physical network under `--no-art`. netd
...
pub const WART_RULE_PREF: u32 = 15000;
...
/// Install the catch-all routing rule (see [`WART_RULE_PREF`]) so unmarked
...
pub fn install_default_rule(ifname: &str) -> io::Result<()> {
    let pref = WART_RULE_PREF.to_string();
