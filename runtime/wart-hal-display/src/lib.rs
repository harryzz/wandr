...
mod binder_path {
    ...
    pub fn set_display_brightness(level: f32) -> bool {
        ...
        let ids = match svc.r#getPhysicalDisplayIds() {
            Ok(v) => v,
            Err(e) => { log::warn!("wart-hal-display: getPhysicalDisplayIds err={e:?}"); return false; }
        };
        let Some(&id) = ids.first() else { log::warn!("wart-hal-display: no physical display"); return false; };
        let token = match svc.r#getPhysicalDisplayToken(id) {
            Ok(Some(t)) => t,
            Ok(None) => { log::warn!("wart-hal-display: null display token"); return false; }
            Err(e) => { log::warn!("wart-hal-display: getPhysicalDisplayToken err={e:?}"); return false; }
        };
        ...
        match svc.r#setDisplayBrightness(&token, &b) {
            Ok(_) => {
                log::info!("wart-hal-display: setDisplayBrightness({level}) ok");
                true
            }
            Err(e) => {
                ...
                log::warn!("wart-hal-display: setDisplayBrightness({level}) err={e:?}");
                false
            }
        }
    }
