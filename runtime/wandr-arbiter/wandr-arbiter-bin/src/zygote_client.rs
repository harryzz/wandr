//! Text-protocol client for wandr-host's zygote socket.
//!
...
/// Default path where wandr-host's zygote listens. Configurable via
/// the `WANDR_ZYGOTE_SOCK` env var for tests / non-standard layouts.
pub const DEFAULT_ZYGOTE_SOCK: &str = "/data/local/tmp/wandr-zygote.sock";

pub fn zygote_sock_path() -> String {
    std::env::var("WANDR_ZYGOTE_SOCK")
...
/// fullscreen one. Used for IME apps such as `wandr.ime.keyboard`.
...
pub fn subscribe_exits() -> Result<UnixStream> {
    ...
    let mut stream = UnixStream::connect(&path)
        .with_context(|| format!("connect {path} — is wandr-host --zygote running?"))?;
