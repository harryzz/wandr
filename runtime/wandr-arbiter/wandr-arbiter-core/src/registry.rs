...
//! responsibility module (`wandr-arbiter-shell`) can own these via `Ctx`, and the
...
mod tests {
    ...
    fn json_round_trips_apps_home_fg() {
        ...
        s.insert_app(app("wandr.launcher", 101));
        s.set_home(Some("wandr.launcher".into()));
        ...
        assert_eq!(alive, 2);
        assert_eq!(t.home(), Some("wandr.launcher"));
