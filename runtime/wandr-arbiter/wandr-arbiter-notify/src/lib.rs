//! wandr-arbiter-notify — the notification module (Signal bg-receipt M3).
//!
//! Owns the active user-notification list in the core [`Store`]. Apps raise
//! notifications through the `wandr:notify/notifier` host import, which the host
...
use wandr_arbiter_core::{ArbiterModule, Ctx, Effect, LaunchKind, Reply};
