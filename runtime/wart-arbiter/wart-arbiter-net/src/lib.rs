//! wart-arbiter-net — the ConnectivityService role (Arbiter Inc., task 88 M1).
//!
//! The arbiter's single owner of *connectivity state* under `--no-art`, mirroring
//! `wart-arbiter-sensors`: the daemon (`wart-net`) owns the mechanism (associate /
//! DHCP / route / DNS via `wart-hal-net`) and reports state up; this module holds
...
mod tests {
    use super::*;
    use wart_arbiter_core::{Effect, Registry, Store};
