//! wart-arbiter-keyguard — the Keyguard/lockscreen module (v1: swipe-to-unlock).
//!
...
use wart_arbiter_core::{ArbiterModule, Ctx, Effect, Event, Reply, Role, PRIMARY_DISPLAY};

/// The keyguard guest's app-id (launched as a `--standalone-overlay-lock` overlay).
const KEYGUARD_APP_ID: &str = "war.keyguard";
...
impl KeyguardModule {
    ...
    fn do_lock(&mut self, ctx: &mut Ctx) -> Reply {
        ...
        let Some(kg) = Self::keyguard_pid(ctx) else {
            return Reply::err("keyguard-not-running (launch war.keyguard first)");
        };
...
mod tests {
    ...
    fn lock_demotes_app_and_shows_keyguard() {
        ...
        seed(&mut store, "war.dioxus.demo", 10, Role::Foreground);
    ...
    fn unlock_restores_the_covered_app() {
        ...
        assert!(eff.iter().any(|e| matches!(e, Effect::Foreground { app_id } if app_id == "war.dioxus.demo")));
    }
