// wandr-hal-display build.rs — codegen the FULL android.gui.ISurfaceComposer
...
fn main() {
    ...
    let vendor = PathBuf::from("../wandr-host/vendor");
...
fn copy_aidl_tree(src: &Path, dst: &Path) {
    if !src.exists() {
        panic!("wandr-hal-display: AIDL source missing: {}", src.display());
    }
