// wandr-framework-shim — the designed framework-shim for `--no-art` operation
// (task 96). Replaces the ad-hoc `wandr-activityms` stub.
//
...
// (vendored under runtime/wandr-host/vendor/; verified 2026-06-07.)
//
...
// correct for wandr (single-user, privileged). NOT a real ActivityManager; later it
...
// proven under --no-art. Built on a-03; launch via wandr-launch (uid system) so it can
// register a system service name. Set WANDR_SHIM_TRACE=1 to log every transaction
...
// Per-transaction tracing — OFF unless WANDR_SHIM_TRACE is set in the environment.
// (Task-94 used it to dissect exactly what the camera HAL asks each stub; in steady
// `--no-art` it is pure log spam.)
static const bool kTrace = (getenv("WANDR_SHIM_TRACE") != nullptr);
...
public:
    ...
    status_t onTransact(uint32_t code, const Parcel& data, Parcel* reply,
        ...
        if (kTrace) {
            ALOGI("WANDR-SHIM-TRACE activity code=%u flags=%u callingPid=%d callingUid=%d",
...
public:
    ...
    status_t onTransact(uint32_t code, const Parcel& /*data*/, Parcel* reply,
                        uint32_t flags) override {
        if (kTrace) {
            ALOGI("WANDR-SHIM-TRACE %s code=%u flags=%u callingPid=%d callingUid=%d",
...
public:
    ...
    status_t onTransact(uint32_t code, const Parcel& data, Parcel* reply,
                        uint32_t flags) override {
        if (kTrace) {
            ALOGI("WANDR-SHIM-TRACE processinfo code=%u flags=%u callingPid=%d callingUid=%d",
                  code, flags, IPCThreadState::self()->getCallingPid(),
                  IPCThreadState::self()->getCallingUid());
        }
        switch (code) {
...
// everywhere = permissive (correct for wandr, a single privileged user). Codes from
...
public:
    ...
    status_t onTransact(uint32_t code, const Parcel& data, Parcel* reply,
                        uint32_t flags) override {
        if (kTrace) {
            ALOGI("WANDR-SHIM-TRACE appops code=%u flags=%u callingPid=%d callingUid=%d",
                  code, flags, IPCThreadState::self()->getCallingPid(),
                  IPCThreadState::self()->getCallingUid());
        }
        switch (code) {
...
public:
    ...
    status_t onTransact(uint32_t code, const Parcel& data, Parcel* reply,
                        uint32_t flags) override {
        if (kTrace) {
            ALOGI("WANDR-SHIM-TRACE package_native code=%u flags=%u callingPid=%d callingUid=%d",
                  code, flags, IPCThreadState::self()->getCallingPid(),
                  IPCThreadState::self()->getCallingUid());
        }
        switch (code) {
...
int main() {
    ...
    if (st != OK) {
        ALOGE("wandr-framework-shim: addService(activity) failed: %d", st);
        return 1;
    }
    ALOGI("wandr-framework-shim: registered 'activity' (IActivityManager)");
    ...
    for (const auto& g : generics) {
        ...
        ALOGI("wandr-framework-shim: addService(%s) = %d", g.name, s);
    }
    ...
    ALOGI("wandr-framework-shim: addService(processinfo) = %d", pis);
    ...
    ALOGI("wandr-framework-shim: addService(appops) = %d", aos);
    ...
    ALOGI("wandr-framework-shim: addService(package_native) = %d", pns);

    ALOGI("wandr-framework-shim: serving");
