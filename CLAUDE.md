...
| 40-ime | Real IME via rsbinder → IMMS — multi-session arc | 🟡 session 3 done 2026-05-27 — registered as IMMS client. (Sessions: 1 = scope/recon, 2 = vendor + first read-only call, 3 = addClient + Bn-side callback servers, 4 = startInputOrWindowGainedFocus + WindowToken + EditorInfo, 5 = IRemoteInputConnection real editor model + Compose PlatformContext.) **Session 1** (committed `fbddb528` 2026-05-27): scope doc `tasks/40-real-ime.md`. **Session 2** (committed `4bbb218a` 2026-05-27): vendored `vendor/aosp-frameworks-base` (shallow + sparse, 17 MB, pinned `android-15.0.0_r36`); stubbed `IInputMethodManager.aidl` + new `--probe-ime` CLI calling `isImeTraceEnabled()`; transport validated. **Session 3** (2026-05-27): stubbed `IInputMethodClient.aidl` (12 oneway slot_NN methods) + `IRemoteInputConnection.aidl` (1 placeholder) — full editor surface deferred to session 5; un-stubbed `addClient(client, inputConn, displayId)` in IMM AIDL; new `ImeClient` + `RemoteInputConn` Bn-side server impls in `ime_impl.rs` (TokioRuntime adapter from `sensors_impl.rs`); new `--probe-ime-addclient` CLI. Device output: `ime: addClient OK — IMMS accepted our non-Activity client registration.` Verified with `dumpsys input_method` showing our process as a `ClientState{... mUid=0 mPid=<our> ...}` entry with our `IRemoteInputConnection$Stub$Proxy` as fallbackInputConnection. addClient has no `@RequiresPermission` annotation; no SELinux relaxation needed. Next: session 4 = `startInputOrWindowGainedFocus(...)` with a fabricated `IBinder windowToken` — biggest open de-risk is whether IMMS validates the token against WMS (recon suggested no). See `tasks/40-real-ime.md` "Session 3 results". |
