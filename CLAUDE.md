...
| 29 | `tasks/29-tooltip-sigill-bisect.md` | Diagnose the Material3 TooltipBox SIGILL on wasi. 80% bisected: trigger is `ClickableElement` Modifier.Node + `BasicTooltipBox` wrapper machinery simultaneously. Step 1 done 2026-05-19 — test #17 ✅ proves the wrapper structure (scope + dead-Popup-slot + DisposableEffect) is INSUFFICIENT; real `BasicTooltipState` machinery is part of the trigger. Step 2 next: ClickableNode instrumentation. Distinct from the popup-overlay DropdownMenu animation bug ([[popup-overlay]]). 🟡 step 1 done |
