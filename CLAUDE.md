...
| 55 | System status bar (top strip) | 🔲 proposal 2026-05-29. The wart "system shell" top strip (clock/battery/notifications) — SystemUI is force-stopped, so "runtime draws everything" (roadmap §6.3). Proposals: A = guest warpkg `war.statusbar` on a top-anchored overlay SF surface (recommended, reuses task 47 machinery); B = host-drawn skia MVP (clock+battery only); C = per-app (rejected). ART-free data: clock (host), battery (sysfs), notifications (wart-native `post-notification` → arbiter "compositor row", NOT NotificationManagerService), cellular out of scope (telephony dropped). Shared foundation with task 56: top/bottom overlay anchor + content insets (`on-insets-changed` host→guest → Compose WindowInsets) + `war:shell` WIT + arbiter routing. v1 = clock+battery. See `tasks/55-status-bar.md`. |
