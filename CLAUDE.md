...
| 44 | WMS window registration for non-Activity processes (path A from task 40 session 6) | 🟡 session 7 done 2026-05-27 — transport ✅, calling-pid gate identified. Multi-session arc, 2-3 weeks estimated. Vendor `IWindowManager` (154 methods, stub all but `openSession`) + `IWindowSession` (42 methods, stub all but `addToDisplay`/`remove`) + Bn-side `IWindow` + `IWindowSessionCallback` + hand-roll `WindowManager.LayoutParams` parcel layout (~30 fields, the multi-day session). **Session 7** (2026-05-27): vendored all 4 WMS AIDLs via the build.rs self-healing slot-stub pattern (programmatic stub generation — 154+42+16 method placeholders); new `wart-host/src/wms_impl.rs` modeled on `ime_impl::session3`; `--probe-wms-opensession` CLI flag. **Outcome**: rsbinder transport to `system_server`'s `window` service works; WMS deserialized our `IWindowSessionCallback` Bn binder; WMS threw `IllegalStateException: Unknown pid=5553 uid=0` (tag `WindowManager`, "Window Manager Crash" — defensive framework-level throw, not a real crash). Means WMS has no `mPidMap` entry for our process because we never went through ATMS. Session 8 must either register pid via ATMS first or bypass via a privileged window type. Sessions 7-11 plan in doc: openSession → addToDisplay → real LayoutParams → focus → verify Gboard via task 40 session 5 probe. **Prerequisite** for task 40 to ship real IME; useful on its own for any future non-Activity feature that needs WMS-tracked focus/visibility. **Not urgent** — in-canvas Compose keyboard works for current needs. See `tasks/44-wms-window-registration.md` Session 7 results. |
...
| 44 | `tasks/44-wms-window-registration.md` | WMS window registration for non-Activity processes (path A from task 40 session 6). Spin-out of task 40's WMS focus gate. Vendor `IWindowManager.aidl` (146 methods, stub all but `openSession`) + `IWindowSession.aidl` (16 methods, stub all but `addToDisplay`) + Bn-side `IWindow` + `IWindowSessionCallback` + hand-rolled `WindowManager.LayoutParams` parcel layout. Sessions 7-11 plan: openSession → addToDisplay → focus → verify Gboard via task 40 session 5 probe. 2-3 weeks estimated. Prerequisite for task 40's real-IME goal. 🔲 scoped, not started. |
