...
| 44 | WMS window registration for non-Activity processes (path A from task 40 session 6) | 🔲 scoped 2026-05-27, not started. Multi-session arc, 2-3 weeks estimated. Vendor `IWindowManager` (146 methods, stub all but `openSession`) + `IWindowSession` (16 methods, stub all but `addToDisplay`/`remove`) + Bn-side `IWindow` + `IWindowSessionCallback` + hand-roll `WindowManager.LayoutParams` parcel layout (~30 fields, the multi-day session). Sessions 7-11 plan in doc: openSession → addToDisplay (empty LayoutParams, expect rejection identifying required fields) → real LayoutParams → focus → verify Gboard via task 40 session 5 probe. **Prerequisite** for task 40 to ship real IME; useful on its own for any future non-Activity feature that needs WMS-tracked focus/visibility. **Not urgent** — in-canvas Compose keyboard works for current needs; start when standalone mode becomes primary or when the in-canvas keyboard's English-only / no-voice / no-emoji limitations become real user pain. See `tasks/44-wms-window-registration.md`. |
