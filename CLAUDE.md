...
| 40-ime | Real IME via rsbinder → IMMS — multi-session arc | 🟡 session 4 done 2026-05-27 — startInputOrWindowGainedFocus lands. (Sessions: 1 = scope/recon, 2 = vendor + first read-only call, 3 = addClient + Bn-side callback servers, 4 = startInputOrWindowGainedFocus + fabricated windowToken, 5 = EditorInfo + IInputMethodSession + IRemoteInputConnection real editor model + Compose PlatformContext.) **Session 1** (`fbddb528`): scope doc. **Session 2** (`4bbb218a`): vendored `vendor/aosp-frameworks-base`, transport via `isImeTraceEnabled()`. **Session 3** (`a8d266f0`): addClient + Bn-side callback servers; non-Activity registration accepted by IMMS, verified in dumpsys ClientState entry. **Session 4** (2026-05-27): stubbed `IRemoteAccessibilityInputConnection.aidl` (1 placeholder oneway method); un-stubbed `slot_11_startInputOrWindowGainedFocus` in IMM AIDL with full 12-arg signature; new `probe_startinput()` in `ime_impl.rs` that creates a windowToken carrier (`BnRemoteInputConnection.as_binder()`), default-constructs `ImeOnBackInvokedDispatcher`, calls the method twice (windowToken=None then Some). Device output for both attempts: `IMMS returned a null InputBindResult ... Session 4 milestone reached.` Discovered rsbinder's `impl_deserialize_for_parcelable` returns `StatusCode::UnexpectedNull` whenever IMMS sends back a null parcelable — wrapped as success since null is a documented IMMS response for non-bind paths. **Both session-4 biggest unknowns resolved in our favor:** (a) WindowToken validation against WMS does NOT block — fabricated IBinder accepted; (b) empty `ImeOnBackInvokedDispatcher` parcelable stub serializes acceptably (non-null marker + 0 field bytes = Java default-path readFromParcel). New `--probe-ime-startinput` CLI flag. Next: session 5 = vendor a real `EditorInfo` parcel layout + parse the non-null `InputBindResult` to extract the `IInputMethodSession` binder + un-stub the editor-side `IRemoteInputConnection` methods (~36) + Compose `PlatformContext.startInputMethod` wiring. See `tasks/40-real-ime.md` "Session 4 results". |
