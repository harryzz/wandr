...
| 47 | IME via dedicated guest app (Path B) | 🔲 scoped 2026-05-27, not started. ~2 weeks. After analyzing IME options (Gboard-via-our-WMS rejected as 3-6 months + reintroduces ART, breaks §9 lock; in-canvas keyboard retired as long-term direction by user decision), settled on a wart-native architecture closely paralleling Android's: arbiter = IMMS, `war.ime.keyboard` warpkg = InputMethodService instance, WIT `war:ime/{ime,input-connection}` = `IInputMethodClient`+`IInputConnection`, skiko `WasiInputMethod` = `InputMethodManager` facade. First-party `war.ime.keyboard` ships first (English QWERTY); voice/emoji/CJK plug in later as additional `.warpkg`s sharing the same stable WIT contract — no protocol changes needed. 5 steps: WIT shape + arbiter routing skeleton (~1-2d) → skiko `WasiInputMethod` adapter (~2d) → first-party `war.ime.keyboard` Compose warpkg (~1w) → InputFlinger focus arbitration + auto-hide (~2-3d) → polish + future-IMEs framing (~2-3d). Six D-locked design decisions: own zygote-forked process; WIT not binder; IME app is regular `.warpkg`; first-party + extensible; arbiter mediates focus via existing task-46-step-4 mechanism; SF z-order MAX-1 / MAX-2 split. See `tasks/47-ime-via-guest-app.md`. |
