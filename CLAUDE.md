...
| 40-ime | Real IME via rsbinder → IMMS — multi-session arc | 🟡 session 2 done 2026-05-27 — IMMS transport validated. (Sessions: 1 = scope/recon, 2 = vendor + first read-only call, 3 = addClient + IInputMethodClient + WindowToken fakery, 4 = IRemoteInputConnection editor model ~30 calls, 5 = Compose PlatformContext integration.) **Session 1** (committed `fbddb528` 2026-05-27): scope doc `tasks/40-real-ime.md` — 5-actor protocol, AIDL inventory (9 interfaces + 7 parcelables target), recon from `dumpsys input_method` showing IMMS tracks non-Activity clients by `(uid, pid, IBinder)` not Activity tokens (encouraging). **Session 2** (2026-05-27): vendored `vendor/aosp-frameworks-base` shallow + sparse-checkout (17 MB working tree, pinned `android-15.0.0_r36`); stubbed `IInputMethodManager.aidl` with 25 `void slot_NN_<orig-name>()` placeholders + kept `isImeTraceEnabled()` at its real transaction code (FIRST_CALL_TRANSACTION + 25), matching task-22 ISurfaceComposer self-healing build.rs pattern; new `wart-host/src/ime_impl.rs` + `--probe-ime` CLI. Device output: `ime: IMMS round-trip OK — isImeTraceEnabled() = false. Transport validated against com.android.internal.view.IInputMethodManager.` No `setenforce 0` needed (`@RequiresNoPermission` method). Pivoted from plan-doc's `getCurrentInputMethodInfoAsUser` to `isImeTraceEnabled` after seeing the real interface — primitive return + no permission gate + zero args gives a cleaner unambiguous signal. Next: session 3 = stop being a passive caller, start as a binder server (IInputMethodClient) + register via addClient. See `tasks/40-real-ime.md` "Session 2 results". |
