...
| 54 | Arbiter death notification + socket cleanup | 🔲 scoped 2026-05-29, not started. ~3-4 h. Closes the steady-state lifecycle gap exposed by an LMK on 2026-05-29: wart-app killed at `oom_score_adj=0` (foreground); arbiter never noticed because it isn't the parent of zygote-forked apps (SIGCHLD only sees own direct children); orphaned control socket left IME visually present but useless for character keys. Fix: zygote → arbiter death-notification protocol over the existing zygote socket (`SUBSCRIBE_EXITS` + push), polling backstop, socket unlink on child exit + startup sweep. See `tasks/54-arbiter-death-notification.md`. |
