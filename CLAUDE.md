...
| 30 | WASI adapter `assert_fail` + wasmtime signal-handler diagnosis (root cause of task 29) | ✅ resolved 2026-05-19. Originally framed as a Kotlin/Wasm `ScopedMemoryAllocator.destroy()` range-propagation bug — **but JetBrains rejected that framing on [KT-86415](https://youtrack.jetbrains.com/issue/KT-86415/) (2026-05-20):** real cause is a use-after-free — the WASI adapter holds canonical-ABI `realloc` memory across Kotlin's aggressive `freeAllComponentModelReallocAllocatedMemory`. See [[kotlin-wasm-scopedmemory-destroy-bug]] (UPSTREAM CORRECTION section). Our `2.4.255-SNAPSHOT` stdlib patch works empirically but is a leak-trade stopgap, not a real fix; KT-86415 = Investigating. wasi adapter fork (`wasmtime-src/`) self-heals on corruption (slim, 65 KB release). ✅ fully closed 2026-05-20 — Step 4 (signal-handler) deferred with re-open criteria; Step 6 (related-bug audit) found no additional retroactive fixes (Tooltip SIGILL was the only bug with this root cause). |
...
| 33 | Boot-model bring-up — run the runtime as a standalone privileged process (no NativeActivity) | 🔲 scoped 2026-05-20 — sub-roadmap for the post-ART display path. 5 ordered steps: (1) standalone-surface spike — non-Activity `su`-process creates a fullscreen `SurfaceControl` from SurfaceFlinger via libgui `SurfaceComposerClient`, EGL one frame [keystone de-risk]; (2) decouple host from NativeActivity/winit-Android; (3) input from InputFlinger; (4) launch mechanism + SystemUI stop; (5) lifecycle / minimal arbiter. See `tasks/33-boot-model-bringup.md` + `post-art-roadmap.md` §11. |
| 34 | KT-86415 real fix — adapter-State use-after-free via a fixed linear-memory partition (Option B) | 🔲 scoped 2026-05-20. The WASI adapter's 64 KB `State` is `cabi_realloc`'d into Kotlin's `ScopedMemoryAllocator` space → UAF. Tier 2 (persistent reallocAllocator + watermark) was tried and **FAILED on device** — `componentModelRealloc` always runs inside an open scope, and the stdlib can't tell long-lived from per-call realloc. Adapter + Kotlin share one linear memory, so the fix is a static partition: reserve `[0,R)` for `State`, Kotlin's allocator starts at `R`. One constant each side, no heuristic, kills UAF + leak. See `tasks/34-kt86415-fixed-partition.md`. |
...
| 34 | `tasks/34-kt86415-fixed-partition.md` | KT-86415 real fix (Option B) — the WASI adapter's 64 KB `State`, `cabi_realloc`'d into Kotlin's `ScopedMemoryAllocator` bump region, is reused after `freeAll` → use-after-free. Records why the 2.4.255 `destroy()`-patch leaks and why the Tier 2 watermark fix FAILED on device (realloc always interleaves inside open scopes; stdlib can't distinguish long-lived from per-call realloc). Fix: static linear-memory partition — `[0,R)` reserved for `State`, Kotlin's allocator starts at `R`; one constant in `MemoryAllocation.kt` + one in the adapter's `State::new`. Kills UAF + leak, no heuristic. Self-contained for a fresh session. 🔲 scoped |
