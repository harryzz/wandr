...
| 57 | System launcher / home | 🔲 proposal 2026-05-29. The home screen / app grid / boot target — without it a standalone device boots to nothing usable (needs adb to launch anything). Distinct from the taskbar (56): launcher = **fullscreen** home app (app grid + clock + static wallpaper), taskbar = persistent overlay strip. Proposals: A = fullscreen `war.launcher` warpkg the arbiter designates as "home" (recommended — no overlay/insets/shim work, cheapest + highest payoff); B = app-drawer overlay only; C = merge into taskbar (rejected). Introduces an arbiter **home-app concept**: boot-foregrounds home, `go-home`, and **fallback-to-home when the fg app dies** (plugs into task 54's `handle_child_exit`). Shares `war:shell/launcher` WIT + `label`/`icon` manifest + app enumeration with task 56. ART-free (app list from install dirs, no PackageManager; static wallpaper, WallpaperManager dropped). See `tasks/57-launcher.md`. |
