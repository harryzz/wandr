...
| 36 | Cross-app dependencies + system components — the multi-package boundary | 🔲 scoped 2026-05-26 (promoted from `scope-cross-app-deps.md`). Extends task 35's installer + loader to walk a `[dependencies]` table — three flavours (host-provided WIT, runtime-bundled system component under `/data/wart/system-apps/`, user-app under `/data/wart/apps/`). Two composition modes: **same-Store** (library-like, cheap, shared GC + crash domain) and **separate-Store** (service-like, host proxy marshals between Stores). Wasmtime stability re-verified 2026-05-26: `Linker::instance` / `substituted_component_type` / `resource` / `instantiate` stable; true lazy linking still unstable (separate-Store + `OnceCell` is the workaround). Cache-key extension records per-dep wasm sha256; any dep update flips A's hash → re-precompile. **Pre-implementation gate (Q6):** same-Store vs separate-Store default + manifest declaration syntax — needs a concrete second component to drive. See `tasks/36-cross-app-deps.md`. |
