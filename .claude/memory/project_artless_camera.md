...
640×480 YUV (`wandr-host --probe-video imagereader`), matching ART's 29 fps. The
...
1. `permission_checker` (IPermissionChecker) — camera-open hang. wandr-activityms GenericStub.
...
**Runtime recipe:** 4 stubs (wandr-activityms) + `/system/bin/sensorservice` (owns
the single-client sensors HAL, registers `sensorservice`) + `wandr-sensormanager`
(HIDL ISensorManager on top). HAL-OWNERSHIP GOTCHA: sensorservice now owns the
sensors HAL → the task-85 `wandr-sensors` daemon (reads the HAL directly for
...
instead), decode path, WIT + wandr-call video track (rtc-rtp VP8 payloader exists).
See [[project_wandr_call]], [[project_artless_sensors]] (task 85), `tasks/93-video-call.md`.
