---
name: reference_wart_wasi_tls_transport
description: wasi-sockets + wasi-tls give wart guests host-delegated network/TLS on wasmtime 45; Signal needs its pinned CA injected via a custom TlsProvider
metadata:
  type: reference
---

Transport de-risk for a Signal-on-wart app (probe: `repros/wasi-tls-probe/`,
...
- **Host must grant network.** `wasmtime-wasi`'s default `SocketAddrCheck` is
  ...
  wart host currently sets neither (only stdio + RO preopens) → guests have zero
...
Bigger Signal picture in [[reference-signal-on-wart-feasibility]]: transport is
...
to wasm. wart guest drive-model (persistent connection vs render-loop) also open.
