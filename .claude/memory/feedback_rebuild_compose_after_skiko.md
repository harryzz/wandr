...
`~/wandr/skiko/skiko/gradlew publishWasmWasiPublicationToMavenLocal`),
...
**Why:** confirmed by `~/wandr/compose-multiplatform-core/BUILD-wasmWasi.md`
...
**How to apply:** run
`~/wandr/scripts/rebuild-compose-wasi-skiko-depend.sh` after every
skiko republish. The script runs against
`~/wandr/compose-multiplatform-core/` and republishes the 9 dep-ordered
...
After the script completes, also rebuild the wandr-app to .wasm +
...
stat -c '%Y %n' ~/wandr/compose-runtime-wasi/build/libs/*.klib
