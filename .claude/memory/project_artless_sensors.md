...
`libwart_sensors_hal.so` (`runtime/wart-sensors/cpp/`, reads
`android.hardware.sensors@1.0::ISensors` directly — the HAL survives ART-off; HIDL=hwbinder
so this hop MUST be C++, rsbinder can't) + Rust daemon `wart-sensors` (dlopens the shim
like wart-host loads libsf_surface; prefers HAL-fused `DEVICE_ORIENTATION` type 27 →
...
report-orientation 0↔3). Wired into run-hybrid-stack --no-art (launched via wart-launch =
...
set (verified via manual `wart-arbiter report-orientation`); the gap is the sensor TRIGGER.
...
package/permission checks) — all multi-app-sandbox machinery wart doesn't have. NOT a
...
per-app connection mgmt / AppOps / privacy / direct channels, irrelevant to wart.
...
consumers (`wart-hal-sensors`/`wart-arbiter-sensors`, reshaped to consume the shim not
