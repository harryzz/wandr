...
description: "Task 73 — modular wart-arbiter-core + WM geometry move (insets/keyboard/orientation host→arbiter). Coded+builds, NOT device-verified. Resume/verify here."
metadata: 
  node_type: memory
  type: project
...
sentinel 65535 (host keeps env 132/150). `wart-arbiter list` works, no
...
**What was built**
- `runtime/wart-arbiter/` → virtual **workspace** (`.cargo/config.toml` stays at
  root; artifact path unchanged → build/deploy scripts untouched). Members:
  - `wart-arbiter-core` (lib): per-display `Store`, full `Event` vocab, `Ctx`
  ...
  - `wart-arbiter-wm` (lib): owns `report-orientation` verb; reacts to
  ...
  - `wart-arbiter-bin`: legacy `match verb` INTACT; `module_owns(verb)` probed
...
- Host (dumb applier; skia dihedral matrix stays local): `InboundEvent::Geometry`
  ...
  their local sensor + `/data/local/tmp/wart-orient-lock` path.

**As-deployed staging (one build; not separately deployable):**
- **Insets = M1** (safe, no change): `run-hybrid-stack.sh` launches the arbiter
  WITHOUT `WART_INSET_*` (only the zygote gets them, line ~145) → WmModule sends
  `INSET_HOST_OWNED` → host keeps its env insets (132/150). **To activate M2**
  (arbiter authors insets): add `WART_INSET_TOP=.. WART_INSET_BOTTOM=..` to the
  `wart-arbiter --daemon` launch env (run-hybrid-stack.sh ~L179/L190).
...
1. `wart-arbiter list` works (legacy match intact). 2. focus an editor → keyboard
...
the arbiter then rotate → host still rotates (local fallback). 4. `wart-arbiter
overlay` → strip geometry unchanged (`overlay_rect` untouched). 5. logcat: watch
`wart-arbiter-wm:` lines + `ime-inbound: unknown verb` (protocol skew).
