...
description: "Keyguard/lockscreen — a Role::Lockscreen surface + war.keyguard guest + wart-arbiter-keyguard module; auto-lock on screen-off, swipe-to-unlock. DONE + device-verified (v1, insecure)."
metadata: 
  node_type: memory
  type: project
...
`wart-arbiter-power` poller emits. Commits: `4591600e` (M1+M2), `059e5977` (M3).
...
- core `Role::Lockscreen` (surface.rs): topmost-but-below-statusbar; excluded from
  `visible_app()`/the task-cycle ring (war.keyguard added to shell `CHROME_APP_IDS`).
...
- `wart-arbiter-keyguard` module: lock state `{locked, saved_fg}`. Auto-locks on
...
- `apps/system/war.keyguard`: light canvas guest (like war.statusbar) — big clock
  (`status::clock-text`) + "swipe up to unlock". Imports `war:keyguard/keyguard`
