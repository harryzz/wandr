...
description: Signal client on wandr is a wasm32-wasip2 GUEST using the generic task-66 wasi:tls transport — NOT a native host daemon (that design was rejected; app logic belongs in the guest)
metadata: 
  node_type: memory
  type: project
...
Building a simple text-only Signal client as a wandr app (task 67). The
...
lives in the guest.** It's just another wandr app, using the generic host
...
connection in a bespoke native daemon (`wandr-signal`, presage-based, mirroring
wandr-arbiter). Discarded. **Why:** app logic belongs in the app; the host stays
...
[[reference_wandr_wasi_tls_transport]], [[feedback_wasi_threading]], and
...
WIT contract — `signal-engine` (exports `wandr:signal/chat`: init/poll-events/send/
