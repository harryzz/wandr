...
the host + Signal — but did NOT rebuild the **IME** (`war.ime.keyboard`) or
**wart-app**. Those crashed at launch with "expected record of 4 fields, found 3"
...
- The skiko-ui consumers as of 2026-06-04: `apps/user/wart-app`,
  `apps/system/war.ime.keyboard`, `apps/user/war.signal/ui`.
- Rebuild a Kotlin/Compose guest: `cd <app> && ./gradlew
  ...
  embed→component-new→`wart-host --install`, NO apps-root wipe).
- **Do NOT** use `build-system-warpkgs.sh` to fix one stale app — it `rm -rf`s the
  apps root and destroys Signal state. See [[feedback_build_system_warpkgs_wipes_apps_root]].
- Diagnose the trap by launching the guest via the arbiter and reading the
  `wart-zygote/child: standalone failed: linker.instantiate failed: …` line in
...
- **skiko's Kotlin WIT binding is HAND-MAINTAINED, not WIT-generated.**
  ...
  guests (IME, wart-app) couldn't `component embed` (core module imported 3
...
- **The wart-zygote PRELOADS guest components in memory — a stack restart is
  ...
  (safe: pkills + restarts wart-host/arbiter, does NOT wipe APPS_ROOT) so the
