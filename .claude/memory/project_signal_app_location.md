...
**ROLE: wandr.signal is the runtime's capability-PROOF demo, not the product.** It
...
cut corners, so "Signal works end-to-end" is the demonstration that wandr can
...
`repros/` into **`apps/user/wandr.signal/`** on 2026-05-31:

- `apps/user/wandr.signal/engine/` — the wasm32-wasip2 engine component exporting
  `wandr:signal/chat` (was `repros/signal-engine`). Path deps are 4-up
  (`../../../../external/...`).
- `apps/user/wandr.signal/ui/` — the dioxus-canvas guest importing `chat` (was
  `repros/signal-ui`).
- `apps/user/wandr.signal/package.toml` — the wandrpkg manifest (was already here).
- `apps/user/wandr.signal/build.sh` — builds both, `wac plug`s ui◁engine, assembles
  `build/wandr.signal.wandrpkg`; `./build.sh --deploy` also installs + relaunches on
