...
> (the HAL survives ART-off) + a Rust `wart-sensors` daemon that prefers the HAL-fused
...
Ran the on-device `/system/bin/sensorservice` via `wart-launch` (uid system) +
...
— but most is per-app connection mgmt / AppOps / privacy / direct channels that wart's
...
- Base sensors (accel, **proximity**, light, gyro, mag) flow straight to the existing
  Rust consumers (`wart-hal-sensors` / `wart-arbiter-sensors` reshaped to consume the
...
Built `wart_sensors_probe` (cc_binary, `android.hardware.sensors@1.0`) and ran it under
`--no-art` via `wart-launch` + `setenforce 0`. **It works**: `ISensors@1.0` acquired, **29
...
- `runtime/wart-sensors/cpp/wart_sensors_hal.cpp` → **`libwart_sensors_hal.so`** — thin
  C-ABI HIDL shim (open / enable-by-type / poll), `dlopen`'d by the daemon (HIDL = hwbinder,
  unreachable from rsbinder). Built on a-03.
- `runtime/wart-sensors/` Rust **`wart-sensors`** daemon — `dlopen`s the shim, prefers
...
- Build the C++ HIDL shim on a-03 (against `android.hardware.sensors@1.0`, libhidlbase)
  → `libwart_sensors.so` (or a small `wart-sensors` reader binary), deploy to device.
- Wire the Rust side (`wart-hal-sensors`/`wart-arbiter-sensors`) to the shim; add the
...
the wart sensor consumers (orientation, proximity) work — no manual `report-orientation`.

## Related
Task 84 (input routing — rotates correctly once orientation is set), task 77
(`wart-arbiter-sensors`, consumes `wart-hal-sensors`), task 78 (proximity screen-off),
