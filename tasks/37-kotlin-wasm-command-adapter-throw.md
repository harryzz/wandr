...
Confirmed on Pixel 2 XL 2026-05-26 via `wart-host --run-once`:
...
- Behavior is identical under:
  ...
  - `wart-host --run-once` on device with `wasi_stderr` routing fd 2
    to logcat — same throw, no stderr output before the trap.
- The *reactor* adapter has different init semantics (`_initialize`
  only, no `_start`) and does NOT trip this — wart-app uses it daily
  with no issues.
...
2. **What's the wasm exception payload?** Kotlin/Wasm uses
   ...
   wart-app code in `lib.rs:354-403` shows how — same recipe applies
...
| 3 | **Bisect Kotlin versions** — try 2.3.x → does the bug exist there? If only 2.4.0-RC, it's a regression worth filing on YouTrack. | rebuild wart-app-md-smoke against older `kotlin("multiplatform") version` |
| 4 | **Bisect adapter versions** — try older `wasi_snapshot_preview1.command.wasm` builds. wart's wasmtime-src fork is at a specific commit; pre-fork versions of the adapter may or may not trip this. | rebuild command adapter from upstream wasmtime tags |
| 5 | **File upstream** — if it's reproducible with stock Kotlin + stock adapter, this is a JetBrains / wasmtime issue worth tracking. Reference the wart-leak-repro pattern (task 24/25) — a minimal Kotlin file that throws under `wasmtime run`. | YouTrack issue + a minimal repro tarball |
...
- `wart-app-md-smoke/` Kotlin consumer's `main()` will run cleanly →
...
- Restructuring wart-host to avoid command-adapter consumers (we already
