...
```
[129767.305329] lowmemorykiller: Killing 'wart-host' (29087) (tgid 29087), adj 0,
[129767.305692] lowmemorykiller: Killing 'binder:29087_6' (29144) (tgid 29087), adj 0,
```

- wart-app PID 29087 (`oom_score_adj=0`, the foreground tier
  set by `wart-arbiter foreground`) was killed by the kernel
  LMK. The IME (29133) survived because its working set is
  smaller (~80 MB vs ~180 MB for wart-app with Compose +
  skiko + cards + assets); LMK tie-breaks by RSS.
- The arbiter's `list` still showed `app=com.example.wart-app
...
- Character keys silently disappeared — arbiter would write
  ...
  `/data/local/tmp/wart-host-29087.sock` (zombie file from the
...
Process tree:
```
pid 29052 wart-host --zygote    PPID=1
pid 29071 wart-arbiter --daemon PPID=1
pid 29133 wart-host (IME)       PPID=29052   ← zygote is parent
pid 29087 wart-host (wart-app)  PPID=29052   ← zygote is parent (now dead)
```
...
`wart-host` children bind `/data/local/tmp/wart-host-<pid>.sock`
...
Accumulated artifact: 50+ stale `wart-host-<old-pid>.sock` files
...
- Arbiter's reader thread parses each line, calls a new
  ...
  - Persists the new state to `wart-arbiter-state.json`.
...
Each `wart-host` child unlinks its `.sock` file at shutdown:

```rust
// runtime/wart-host/src/ime_inbound.rs::start_listener
let path = format!("/data/local/tmp/wart-host-{}.sock", pid);
...
Plus a startup sweep: `wart-host --zygote` deletes any
`wart-host-*.sock` whose `<pid>` is not in the current process
...
| # | Step | Estimate |
|---|---|---|
| 1 | Extend zygote socket protocol — `SUBSCRIBE_EXITS` request, broadcast `EXITED <pid> <signal>` to subscribers from SIGCHLD reaper. `runtime/wart-host/src/zygote.rs`. | ~1 h |
| 2 | Arbiter subscriber thread — opens connection on daemon startup, reconnects on disconnect (with backoff), feeds events into existing state-mutation path. `runtime/wart-arbiter/src/main.rs` + `zygote_client.rs`. | ~45 min |
| 3 | `state::on_child_exit(pid, signal)` — removes from running map; cascades to focused-pid / active-ime cleanup; signals IME demote if needed; persists state. `runtime/wart-arbiter/src/state.rs`. | ~45 min |
| 4 | Polling backstop (5 s `kill(pid, 0)`) — separate thread, same `on_child_exit` callback. | ~20 min |
| 5 | Socket unlink — RAII guard in `ime_inbound::start_listener` + startup sweep in `zygote::serve`. `runtime/wart-host/src/{ime_inbound,zygote}.rs`. | ~30 min |
| 6 | Device verification — reproduce the OOM-kill scenario (`adb shell stop` then trigger memory pressure, OR `am send-trim-memory` if available, OR just `kill -9 <wart-app-pid>` from `su`), confirm: (a) arbiter `list` updates within 5 s, (b) IME demotes / clears editor focus, (c) `.sock` file removed on graceful exit and surviving stale files swept at next zygote start. | ~30 min |
...
adb shell "su -c '/data/local/tmp/wart-arbiter launch-overlay war.ime.keyboard'"
adb shell "su -c '/data/local/tmp/wart-arbiter set-ime war.ime.keyboard'"

# 2. Simulate LMK
WART_APP_PID=$(adb shell "ps -ef | grep wart-host | awk 'NR==3{print \$2}'")
adb shell "su -c 'kill -9 $WART_APP_PID'"

# 3. Within 5 s arbiter list should drop wart-app + clear fg
adb shell "su -c '/data/local/tmp/wart-arbiter list'"
# Expected:
#   OK count=1
#     app=war.ime.keyboard pid=<x> elapsed_ms=… [ime]
...
- Restart-on-crash policy (auto-relaunch wart-app if killed) —
...
- [`tasks/45-wart-zygote-spike.md`](45-wart-zygote-spike.md) —
  the zygote design that established zygote-as-parent.
- [`tasks/46-wart-arbiter-mvp.md`](46-wart-arbiter-mvp.md) —
