...
>   2. Languages should ship as installable plugins (`wandr.lang.fr`,
>      `wandr.lang.de`, …) so adding a new language doesn't require
...
> `wit/skiko-gfx.wit` and a new `wandr-host/src/ime_outbound.rs`
...
>     `wandr.ime.keyboard`'s WIT just needs to `export wandr:ime/ime`.
>   - `wandr-host/src/ime_inbound.rs` (shipped in step 3a) ALREADY has
...
>     socket file (`/data/local/tmp/wandr-host-<pid>.sock`), same
...
> exported `wandr:ime/ime`, and (b) the keyboard-lang plugin contract.

## Why this task exists

Today `wandr.ime.keyboard` shows the English QWERTY for every focused
...
wandrpkg, and pushing — a heavy operation for what should be a single
file drop.
...
…and wandr-app forwards an `EditorInfo` to the arbiter via
...
Symbols2, Emoji). Languages ship as plugins (`wandr.lang.bg`,
`wandr.lang.fr`, …). Bulgarian moves OUT of built-ins as the first
...
task ships Stage 1 (wandr.ime.keyboard's `package.toml` declares
which languages it supports). Stage 2 (the arbiter exposes
`list-installed wandr.lang.*` and the IME picks them up at startup
...
`/data/local/tmp/wandr-host-<pid>.sock` (opened by `ime_inbound.rs`
...
**D7. Wandr-app's hardcoded `input-type="text"` is NOT a one-line
...
The file already opens `/data/local/tmp/wandr-host-<pid>.sock` in
...
- `input-type` is the bare enum-tag string (`text` / `number` /
  `phone` / …) — same set as `wandr:ime/input-type`.
...
- `cmd_attach_editor` already validates + stores `EditorFocus`.
  ...
  (`/data/local/tmp/wandr-host-<ime-pid>.sock`). Reuses the
  `deliver_to_host` one-shot-connect helper already in main.rs.
- `cmd_detach_editor` writes `editor-detached`.

**1b. Host→guest bindings for `wandr:ime/ime`:**

The `wandr:ime/ime` interface (`on-editor-attached(info)` /
...
guest's `wandr:ime/ime`, two options:
...
- **Option A — Second `bindgen!` macro.** Add:
  ...
  mod ime_bindings {
      wasmtime::component::bindgen!({
          path: "../wit/",
          world: "wandr:ime/ime-client-world",
      });
  }
  ```
  Generates typed `EditorInfo` struct + typed `call_on_editor_attached`.
  Cleaner; matches the existing skiko-ui bindgen pattern. The
  guest must be linked against that world, so `wandr.ime.keyboard`'s
  `wit/wandr-ime-keyboard.wit` needs to `include` `ime-client-world`
...
- **Option B — Untyped `Val` calls.** Look up the export at
  ...
  ```rust
  let ime_iface = instance.get_export_index(&mut store, None, "wandr:ime/ime@0.1.0").unwrap();
...
**Wandr-app side: nothing changes.** wandr-app already calls
...
**`wandr.ime.keyboard` side: export the interface.**

- `wandr.ime.keyboard/wit/wandr-ime-keyboard.wit` — add
  `export wandr:ime/ime;` (already has `include my:skiko-gfx/skiko-ui`).
  May need to restructure to be `world ime-keyboard { include
  my:skiko-gfx/skiko-ui; include wandr:ime/ime-client-world; }`
  or split the export off — verify during impl.
- `wandr.ime.keyboard/wit/deps/ime/ime.wit` — mirror of
  `wit/ime.wit`.
- `wandr.ime.keyboard/src/wasmWasiMain/kotlin/generated/SkikoUi.kt`
  (or a new file): hand-add `@WasmExport` extern declarations
  for `wandr:ime/ime.on-editor-attached` and `on-editor-detached`.
...
**Success criterion:** `wandr-arbiter attach-editor <wandr-app-pid>
...
- **`wandr.ime.keyboard/src/wasmWasiMain/kotlin/ImeKeyboard.kt`** — new
  built-in layouts:
...
- **`wandr-app/src/wasmWasiMain/kotlin/WasiKeyboardController.kt`** —
  ...
  is a multi-file refactor across `wandr-app/.../wasmWasiMain/`
  ...
  wandr-app's demo so the path is exercised on the smoke.

**Success criterion:** tap a Number TextField in wandr-app → IME
...
The `wandr:keyboard-lang/lang` WIT package.

- **`wit/keyboard-lang.wit`** (new):
  ```wit
  package wandr:keyboard-lang@0.1.0;
...
- `wit/keyboard-lang.wit` written, validated with `wasm-tools component
  wit`. Mirrored to `wandr.ime.keyboard/wit/deps/keyboard-lang/`. Not
  mirrored to `wandr-app/wit/deps/` — wandr-app doesn't import the
  contract.
- `wandr.lang.bg/` Rust cdylib added: `Cargo.toml` + `src/lib.rs`
  (~60 LoC). Exports `wandr:keyboard-lang/lang@0.1.0` with `get-info` →
  ...
  `export wandr:keyboard-lang/lang@0.1.0`.
- `wandr.ime.keyboard` updated:
...
- The `scripts/build-system-wandrpkgs.sh` integration is deferred to step
...
### Step 4 — Sample language plugin: wandr.lang.fr (~1 h)

New sibling repo `wandr.lang.fr/` (Rust cdylib). Static AZERTY data.
...
- `package.toml`:
  ```toml
  app_id      = "wandr.lang.fr"
  version     = "0.1.0"
  world       = "wandr:keyboard-lang/lang-world"
  kind        = "system"
...
- **`scripts/build-system-wandrpkgs.sh`** — add steps to build / package
  / push / install the new language wandrpkgs (wandr.lang.bg from
  step 3 and wandr.lang.fr from this step).

**Success criterion:** `wandr-host --install wandr.lang.fr.wandrpkg`
succeeds; `<APPS_ROOT>/system-apps/wandr.lang.fr/0.1.0/` contains
the precompiled cwasm.

#### Step 4 results

- `wandr.lang.fr/` Rust cdylib added — sibling of `wandr.lang.bg/`,
...
- Build: `cargo build --target wasm32-wasip2 --release` → 26 s.
  ...
  `export wandr:keyboard-lang/lang@0.1.0`.
- `scripts/build-system-wandrpkgs.sh` extended with `FR_WASM` /
  `FR_PKG` / `pack_wandrpkg` block / push + install loop entry —
  pattern-identical to the wandr.lang.bg additions in step 3.
- Step 5 declares both plugins as IME `[dependencies]`; the
  device 🌐 cycle becomes English → Bulgarian → French.

### Step 5 — Dynamic loading in wandr.ime.keyboard (~1 h)

The IME enumerates declared lang deps + loads them via the existing
generic dep wiring.

- **`wandr.ime.keyboard/package.toml`** — add `[dependencies]`:
  ```toml
  [dependencies]
  bg = { system = "wandr.lang.bg", version = "0.1.0", interface = "wandr:keyboard-lang/lang@0.1.0" }
  fr = { system = "wandr.lang.fr", version = "0.1.0", interface = "wandr:keyboard-lang/lang@0.1.0" }
  ```

- **`wandr.ime.keyboard/src/wasmWasiMain/kotlin/LangAdapter.kt`** (new) —
  the bridge:
...
- **`wandr.ime.keyboard/src/wasmWasiMain/kotlin/ImeKeyboard.kt`** —
...
- **Plumbing the wandr-host generic dep wiring** — task 39 already
  registers proxy closures via `LinkerInstance::func_new`. No
  changes to wandr-host. The IME's wasm just declares the imports;
...
- **Per-plugin WIT packages.** Two deps cannot share the same
  ...
  doc's literal `interface = "wandr:keyboard-lang/lang@0.1.0"`
  collision is resolved by giving each plugin its own WIT package:
  `wandr:keyboard-lang-bg/lang@0.1.0` (in `wandr.lang.bg/wit/keyboard-lang-bg.wit`),
  `wandr:keyboard-lang-fr/lang@0.1.0` (in `wandr.lang.fr/wit/keyboard-lang-fr.wit`).
  The canonical schema stays in `wandr/wit/keyboard-lang.wit` as a
...
- **`scripts/pack-ime-keyboard.sh`** — new dedicated packing script
  for wandr.ime.keyboard. The IME's `package.toml` declares both
  langs as `[dependencies]`.
- **Device verification (2026-05-28)** —
  ```
  loader: loaded dep `bg` (wandr:keyboard-lang-bg/lang@0.1.0) from …
  loader: loaded dep `fr` (wandr:keyboard-lang-fr/lang@0.1.0) from …
...
- **Pivots 1 (regular → English), 2 (Number → Numeric, 🌐 hidden),
  ...
  tap` injection didn't reach wandr-app's BasicTextField cleanly
  on this device today (unrelated to task 49 — likely focus
  routing on the launcher-vs-wandr-app boundary; the in-app
...
| Repo | File(s) | Why |
|---|---|---|
| **wandr** | `wit/keyboard-lang.wit` (new) | Plugin contract for lang plugins |
| **wandr** | `tasks/49-ime-content-control.md` | This file, with results section per step |
| **wandr** | `scripts/build-system-wandrpkgs.sh` | Build / package the new lang wandrpkgs |
| **wandr-host** | `src/ime_inbound.rs` | Extend with `editor-attached` / `editor-detached` parsing + new `InboundEvent` variants |
| **wandr-host** | `src/standalone.rs` | New drain arms calling IME's `on-editor-attached` / `on-editor-detached` |
| **wandr-host** | `src/lib.rs` | Second `bindgen!` macro for `wandr:ime/ime-client-world` (Option A) OR untyped `Val` calls in standalone.rs (Option B) |
| **wandr-arbiter** | `src/main.rs` | `cmd_attach_editor` / `cmd_detach_editor` write to IME's per-host socket via existing `deliver_to_host` helper |
| **wandr-app** | `src/wasmWasiMain/kotlin/WasiKeyboardController.kt` | `pendingKeyboardType` field + `show()` reads it; per-D7 MVP hack |
| **wandr-app** | `src/wasmWasiMain/kotlin/RealComposeApp.kt` | TextFieldCard sets `pendingKeyboardType` in `.onFocusChanged` |
| **wandr-app** | demo screens add Number + Phone TextFields | Smoke coverage |
| **wandr.ime.keyboard** | `src/wasmWasiMain/kotlin/ImeKeyboard.kt` | New built-in layouts (Numeric / Phone / Email / Url / Password) + pickLayout |
| **wandr.ime.keyboard** | `src/wasmWasiMain/kotlin/LangAdapter.kt` (new) | Hand-written WIT bindings + loader for lang plugins |
| **wandr.ime.keyboard** | `src/wasmWasiMain/kotlin/generated/SkikoUi.kt` (or new file) | `@WasmExport` declarations for `wandr:ime/ime.on-editor-*` (the EXPORT side — needs canonical-ABI lift for `editor-info` record) |
| **wandr.ime.keyboard** | `src/wasmWasiMain/kotlin/RealComposeApp.kt` | Wire the new exported `on-editor-attached` → updates `currentEditorType` MutableState |
| **wandr.ime.keyboard** | `package.toml` | `[dependencies]` for lang plugins |
| **wandr.ime.keyboard** | `wit/wandr-ime-keyboard.wit` | `import keyboard-lang` + restructure world to export `wandr:ime/ime` |
| **wandr.ime.keyboard** | `wit/deps/ime/ime.wit` (new mirror) | Mirror of `wandr/wit/ime.wit` |
| **wandr.ime.keyboard** | `wit/deps/keyboard-lang/keyboard-lang.wit` (new) | Mirror |
| **wandr.lang.bg** (new repo) | full Rust cdylib | Bulgarian, extracted from built-ins |
| **wandr.lang.fr** (new repo) | full Rust cdylib | French AZERTY |

**Not touched (corrections from v1):**

- `wit/skiko-gfx.wit` + 3 mirrors — no changes. v1 proposed a new
  `ime-events` interface; that's redundant with `wandr:ime/ime`.
- `wandr-host/src/ime_outbound.rs` — no new file. The existing
...
- `skiko/.../generated/{Internal,}SkikoUi.kt` — no `ime-events`
  ...
  `wandr.ime.keyboard`'s own generated files, NOT in skiko's
  shared bindings).
...
- **Task 49 owns the wire format.** The `editor-attached` /
  ...
  `ime_inbound.rs`. The host→guest bindings for `wandr:ime/ime`.
...
piggy-back on the `wandr:ime/ime` interface — either by adding a
...
- **Hand-written Kotlin WIT bindings are tedious.** The `lang`
  ...
  `wandr-app/.../MarkdownImports.kt` — the markdown renderer has
  the same shape (records with strings in lists) and the lift
  pattern carries over.

- **The `wandr:ime/ime` EXPORT side is harder than typical IMPORTs.**
...
- **Layout-pick order matters when both fire.** If
  ...
  event lands in the wandr-host's drain queue regardless of
...
- **Wandr-host needs to know which surface is the IME.** Today
...
- **Wandr-app's hardcoded `inputType="text"` is the MVP hack
...
- **Stage 2: dynamic discovery.** Arbiter exposes
  `list-installed wandr.lang.*`; the IME builds its lang list at
  startup without manifest declarations. ~2-3 hours of cleanup on
  top of task 49.

- **Layouts as plugins too.** Numeric / Phone / Email could be
  refactored into `wandr.layout.numeric` etc. Not done in task 49 —
...
- `feedback_softkeyboard` — the in-canvas keyboard (now retired
  from wandr-app's regular path); the layout patterns came from
  there.
...
1. **Read the revision notes at the top.** v1 of this doc
   ...
   `wandr:ime/ime` (from `wit/ime.wit` shipped in task 47 step 1)
   and extend `wandr-host/src/ime_inbound.rs`.
2. **Step 1 has two sub-layers.** Don't conflate them: 1a is
   socket-level message parsing (~30 min); 1b is wasmtime
   host→guest binding for `wandr:ime/ime` (~1.5 h). Pick Option A
...
6. **wandr-app's Number/Phone TextFields are the device smoke
