...
- **`runtime/wandr-hal-display/`** — NEW shared crate (mirrors `wandr-hal-sensors`):
  ...
  `setPowerMode(token, ON/OFF)`. `ensure_process_state()` like wandr-hal-sensors.
- **core** (`wandr-arbiter-core`): `Effect::SetDisplayPower { on }`.
- **`wandr-arbiter-power`**: `blanked` flag; on `ProximityChanged` blank only while
...
- **`wandr-arbiter-bin`**: `Effect::SetDisplayPower` arm in `execute_effects` →
...
- Device: `audio-call-start wandr.signal` → proximity auto-enabled (task-77 wiring);
...
- Adopting `wandr-hal-display` in the host's `display_impl.rs`; auto-brightness via
