# Task 90 — implement the `war:connectivity` WIT (guest-facing connectivity)
...
> (`wit/connectivity.wit`, commit `6a36f2b1`) but **not wired into wart-host** — no
...
- **`runtime/wart-arbiter/wart-arbiter-net`** — status of record (`net-status`)
...
- **`runtime/wart-net` + `runtime/wart-hal-net`** — the daemon + HAL binder
  clients already drive `IWifi`/`ISupplicant`/`INetd`/`IDnsResolver` (uid system).
- **`runtime/wart-host`** — guests already reach the network via `wasi:sockets`
  (`inherit_network`); this task adds the *control* WIT, not a data path.

## The pattern to mirror

`war:alarm` is the canonical arbiter-facing WIT host path — copy it:
- `runtime/wart-host/src/lib.rs` — `bindgen!` the `*-host` + `*-events` worlds.
- `runtime/wart-host/src/alarm_host_impl.rs` — `impl Host` forwarding to the
  arbiter socket (`crate::arbiter_sock`); `runtime/wart-host/src/app_loader.rs`
...
1. `wart-host`: `bindgen!` `connectivity-host` + `connectivity-events`; new
...
DoD: a guest's `on-connectivity-change` fires when wart-net flips the link.

### M2 — `wifi` scan + connect (privileged)
1. **`scan`** — bind **`IWificond`** (`android.net.wifi.nl80211.IWificond`,
   service `wifinl80211`, survives `--no-art`) in `wart-hal-net` (same rsbinder
   pattern as `IWifi`/`ISupplicant`); trigger a scan + return `scan-result`s.
   New daemon verb / `wart-net --scan`; arbiter relays.
...
1. **Credential storage decision** (Open question below) — a wart-owned store vs
   the framework's `WifiConfigStore.xml`. Build the chosen store.
2. `wart-arbiter-net` becomes the **WifiConfigManager**: saved-network registry,
...
3. `wart-net` auto-joins a known network at bring-up from the store (replacing the
...
- A `war.settings.wifi` guest (separate warpkg) consuming `wifi-settings`.

## Open questions (decide before M3)
1. **Credential storage** (`--no-art`): wart-owned encrypted store, vs reuse
...
- `war:alarm` host wiring (the pattern) + `docs/architecture-host-guest-boundary.md`.
- `[[feedback_read_source_first]]` — read the AIDL (`IWificond`) + the `war:alarm`
