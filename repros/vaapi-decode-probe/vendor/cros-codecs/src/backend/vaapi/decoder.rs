...
impl<V: VideoFrame> VaapiBackend<V> {
    pub(crate) fn new(display: Rc<libva::Display>, supports_context_reuse: bool) -> Self {
        ...
        // ── wandr patch ──────────────────────────────────────────────────────
        // The throwaway placeholder context must satisfy the DRIVER's MINIMUM
        // decode resolution. Upstream hardcodes 16x16, which i965/iHD accept but
        // Mesa's D3D12/VAOn12 driver rejects with
        // VA_STATUS_ERROR_RESOLUTION_NOT_SUPPORTED (19) -- it enforces a decode
        // heap minimum. Ask the driver instead of assuming a constant.
        /// Only used when the driver reports NO minimum: the smallest size
        /// observed to satisfy D3D12/VAOn12's decode-heap minimum while staying
        /// trivially cheap to allocate.
        const PLACEHOLDER_FALLBACK_DIM: u32 = 256;
        fn min_dim(cfg: &mut libva::Config, t: libva::VASurfaceAttribType::Type) -> u32 {
            cfg.query_surface_attributes_by_type(t)
                .ok()
                .and_then(|v| {
                    v.into_iter().find_map(|g| match g {
                        libva::GenericValue::Integer(i) if i > 0 => Some(i as u32),
                        _ => None,
                    })
                })
                .unwrap_or(PLACEHOLDER_FALLBACK_DIM)
        }
        let min_w = min_dim(&mut config, libva::VASurfaceAttribType::VASurfaceAttribMinWidth);
        let min_h = min_dim(&mut config, libva::VASurfaceAttribType::VASurfaceAttribMinHeight);
        log::info!("wandr: VA placeholder context {min_w}x{min_h} (driver-reported minimum)");
        let init_stream_info = StreamInfo {
            format: DecodedFormat::NV12,
            coded_resolution: Resolution::from((min_w, min_h)),
            display_resolution: Resolution::from((min_w, min_h)),
            min_num_frames: 1,
        };
        let context = display
            .create_context::<<V as VideoFrame>::MemDescriptor>(
                &config, min_w, min_h, None, true,
            )
