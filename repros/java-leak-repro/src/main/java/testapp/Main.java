...
// driver.
//
// Each iteration allocates one short-lived `Frame` (mirroring the Kotlin
// reproducer's per-suspendCoroutine continuation / state-machine instance),
// reads it, then drops it -- so it becomes unreachable WasmGC garbage. With
// wasmtime's DRC collector nothing sweeps until a GC-heap grow fails, so the
// garbage piles up and each successive sweep walks an ever-larger
// over-approximated-roots list.
//
