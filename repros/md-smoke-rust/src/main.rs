...
//
// Imports `wandr:markdown/renderer@0.1.0` (a system-bundled component
// installed under `/data/wandr/system-apps/`), calls its `render` export
// with a hardcoded CommonMark string, and exits 0 if the resulting
// document has at least one block — non-zero otherwise.
//
// Exists because the Kotlin smoke (`wandr-app-md-smoke/`) hits a
...
wit_bindgen::generate!({
    // `consumer` world imports wandr:markdown/renderer (the producer side
...
fn main() {
    ...
    // Print via stderr (routed to logcat by wandr-host's wasi_stderr
