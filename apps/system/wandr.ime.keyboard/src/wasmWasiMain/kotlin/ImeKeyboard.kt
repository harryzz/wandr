// wandr.ime.keyboard — task 47 step 3c follow-up. Port of wandr-app's
...
// languages load at startup from `wandr.lang.*` plugins via the
// `wandr:keyboard-lang/lang` WIT contract (task 49 step 5). Shift +
...
// winit branch sends through `on-key-event-v2` (see wandr-host
...
object ImeKeyboardDefaults {
    ...
    val English: KeyboardLayout = KeyboardLayout(
        ...
        rows = listOf(
            ...
            listOf(
                ...
                space.copy(display = "English", width = 3.4f),
                text("."),
                enter,
                hide,
            ),
    ...
    // Bulgarian + French moved out to `wandr.lang.bg/` + `wandr.lang.fr/`.
    // They load at IME startup via the `wandr:keyboard-lang-*/lang`
        ...
        fun wrap(rows: List<List<KeyDef>>, bottomPunct: KeyDef): List<List<KeyDef>> {
            ...
            return buildList {
                ...
                add(listOf(
                    ...
                    KeyDef("🌐",  KeyAction.CycleLanguage,            width = 1f),
                    KeyDef("😀",  KeyAction.SwitchLayout("Emoji"),    width = 1f),
                    // Show the current language name on the space bar (task: IME polish).
                    space.copy(display = name, width = 3.4f),
                    bottomPunct,
                    enter,
                    hide,
                ))
...
/** Digits + common punctuation. */
     ...
     *  (English only — additional languages load from `wandr.lang.*`
