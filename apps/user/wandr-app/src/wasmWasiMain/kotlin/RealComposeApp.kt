...
private fun MaterialDemoApp() {
    ...
    // `get-accent-color` (sysprop `persist.sys.wandr.accent` — set via
    // `setprop persist.sys.wandr.accent 0xFF34A853`). Auto → dark
    // (the historical default for wandr-app). All reads via remember
            ...
            Box(
                ...
                Column(
                    ...
                    HapticScope(enabled = hapticEnabled) {
                        // (The launcher is its own wandrpkg now — wandr.launcher,
                        // task 57. wandr-app is a plain demo app again; no
                        // launcher card here.)
                        CounterCard()
                        PlayToneCard()
                        CheckboxRadioCard()
                        ...
                        // focus; wandr.ime.keyboard switches to
                ...
                // separate process (wandr.ime.keyboard) with its own
