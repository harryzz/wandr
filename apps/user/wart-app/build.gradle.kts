...
kotlin {
    ...
    sourceSets {
        wasmWasiMain.dependencies {
            // In-tree compose wasi klibs, published per
            // external/compose-multiplatform-core/BUILD-wasmWasi.md (Step 3 +
            // the Step 4 "consuming" pattern). These are the canonical
            // `org.jetbrains.compose.*:*-wasm-wasi:9999.0.0-SNAPSHOT` modules
            // built from external/compose-multiplatform-core — NOT the old
            // discarded out-of-tree `compose-*-wasi:0.0.0-wasi-local` fat
            // bundles (those srcDir a stale out-of-tree compose checkout).
            // Republish one module after editing it via Step 5, e.g.
