...
use crate::exports::wandr::signal::chat::{
...
// wandr-arbiter-audio Ringer + comms session (host-forwarded to the arbiter).
use crate::wandr::audio_focus::focus;
...
macro_rules! send_signals {
    ($sender:expr, $peer:expr, $sigs:expr) => {{
        if let Ok(uuid) = Uuid::parse_str(&$peer) {
            ...
            for sig in $sigs {
                use wandr_call::signal::CallSignal as CS;
                let kind = match &sig {
                    CS::Offer { .. } => "Offer",
                    CS::Answer { .. } => "Answer",
                    CS::Ice { opaque, .. } => {
                        ...
                        if let Ok(Some(line)) =
                            wandr_call::signal::decode_ice_candidate(opaque)
                        {
...
fn to_wit_call_state(s: wandr_call::signal::CallState) -> CallState {
    use wandr_call::signal::CallState as W;
    match s {
...
async fn fetch_turn(push: &PushService) -> Option<wandr_call::turn::TurnConfig> {
                    ...
                    if let Some(content) = content {
                        ...
                        if let ContentBody::CallMessage(cm) = &content.body {
                            ...
                            let kinds: Vec<&str> = call::call_message_to_signals(cm)
                                .iter()
                                .map(|s| match s {
                                    wandr_call::signal::CallSignal::Offer { .. } => "Offer",
                                    wandr_call::signal::CallSignal::Answer { .. } => "Answer",
                                    wandr_call::signal::CallSignal::Ice { .. } => "Ice",
                                    wandr_call::signal::CallSignal::Hangup { .. } => "Hangup",
                                    wandr_call::signal::CallSignal::Busy { .. } => "Busy",
                                })
                                .collect();
                ...
                // before the message outbox so audio stays low-latency.
                {
                    ...
                    if call_engine.is_active() {
                        ...
                        if let Some(st) = st {
                            ...
                            if st == wandr_call::signal::CallState::Connected && !comm_started {
                                focus::call_start();
                                comm_started = true;
                            }
                        }
