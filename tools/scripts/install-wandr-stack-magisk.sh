#!/usr/bin/env bash
# Install the wandr-stack Magisk module to /data/adb/modules/wandr-stack/.
# Reversible — touch the disable / remove flag on device to back out
# without an adb push. See wandr-stack-magisk/README.md.
#
...
echo "▸ pushing module to /data/local/tmp/wandr-stack-stage …"
adb shell 'rm -rf /data/local/tmp/wandr-stack-stage'
adb push "$MOD_SRC" /data/local/tmp/wandr-stack-stage >/dev/null
adb shell 'chmod 755 /data/local/tmp/wandr-stack-stage/*.sh'

echo "▸ moving into $MOD_DST …"
adb shell "su -c '
    rm -rf $MOD_DST
    mv /data/local/tmp/wandr-stack-stage $MOD_DST
    chown -R 0:0 $MOD_DST
...
echo "  remove on next boot:  scripts/uninstall-wandr-stack-magisk.sh"
echo "  tail logs:   adb shell tail -f /data/local/tmp/wandr-stack.log"
