// ITextShaperImpl — Avalonia.Skia 11.3.17 TextShaperImpl with the typeface
// cast retargeted to WandrGlyphTypeface (same HarfBuzzSharp shaping; the
// native calls resolve statically against the wasi harfbuzz archive).
...
using HbGlyphInfo = HarfBuzzSharp.GlyphInfo;

internal class WandrTextShaper : ITextShaperImpl
{
    private static Buffer? s_buffer;
    private static readonly Dictionary<int, Language> s_cachedLanguage = new();
    ...
    public ShapedBuffer ShapeText(ReadOnlyMemory<char> text, TextShaperOptions options)
    {
        ...
        // HarfBuzz needs the surrounding characters to correctly shape.
        ...
        {
            language = new Language(usedCulture);
            s_cachedLanguage[usedCulture.LCID] = language;
        }
        buffer.Language = language;

        var font = ((WandrGlyphTypeface)typeface).Font;
        ...
        for (var i = 0; i < bufferLength; i++)
        {
            ...
            shapedBuffer[i] = new Avalonia.Media.TextFormatting.GlyphInfo(
                glyphIndex, glyphCluster, glyphAdvance, glyphOffset);
        }
    ...
    private static void MergeBreakPair(Buffer buffer)
    {
        ...
        if (length > 1 && glyphInfos[length - 2].Codepoint == '\r' && second.Codepoint == '\n')
        {
            ...
            unsafe
            {
                fixed (HbGlyphInfo* p = &glyphInfos[length - 2]) { *p = first; }
                fixed (HbGlyphInfo* p = &glyphInfos[length - 1]) { *p = second; }
            }
        }
        else
        {
    ...
    private static Vector GetGlyphOffset(ReadOnlySpan<GlyphPosition> glyphPositions, int index, double textScale)
    {
        ...
        return new Vector(position.XOffset * textScale, -position.YOffset * textScale);
    }

    private static double GetGlyphAdvance(ReadOnlySpan<GlyphPosition> glyphPositions, int index, double textScale)
        => glyphPositions[index].XAdvance * textScale;
