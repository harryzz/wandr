...
> Scope: how the wandr stack reaches hardware sensors with the Java framework
...
with **one** client = sensorservice). The old direct-HAL opener `wandr-sensors` is
...
```
                   sensors HAL @1.0 (single-client)  ← only sensorservice opens it
                       ...
                       /system/bin/sensorservice  (the multiplexer + data source)
                              │
                wandr-sensormanager (one process, two façades)
       ┌──────────────────────┴───────────────────────────────┐
  ...
  (via HIDL)                                  ┌────────────┴──────────────┐
                                       arbiter sensor_driver        each wandr-host
...
sensorservice binder), so wandr-sensormanager is *not* a HAL owner — it requires the
standalone sensorservice running.

## Cross-check vs the standard Android ways to reach sensors

| Android mechanism | Under `--no-art` | Used by wandr? |
|---|---|---|
...
| libsensor `android::SensorManager` (native) | works | yes — inside both wandr-sensormanager façades |
| frameworks `ISensorManager` **HIDL** @1.0 | needs a publisher (system_server gone) | yes — wandr-sensormanager publishes it; camera EIS consumes |
| frameworks `ISensorManager` **AIDL** | needs a publisher | yes — wandr-sensormanager publishes; arbiter + hosts consume via `wandr-hal-sensors` |
| sensors HAL direct (`ISensors` HIDL/AIDL) | single-client | **only** sensorservice (✓ `wandr-sensors` removed) |
...
task-43 path was left in place: `wandr-host/src/standalone.rs` still enabled
...
to the foreground editor/app + fans overlays. Removed from `wandr-host/src`:
...
`cargo build --target aarch64-linux-android --release` of wandr-host passes.
...
- Apply path: `wandr-arbiter report-orientation 1` → arbiter decided `orient=4` and
...
thread's scheduling can't affect gyro timing. `wandr-sensormanager` now **drops
...
File: `runtime/wandr-sensormanager/cpp/wandr_sensormanager.cpp` (`drop_rt_scheduling_capability`).

**Device-verified (2026-06-07)** after a fresh `--no-art` bringup with the rebuilt
binary: logcat `wandr-sensormanager: dropped CAP_SYS_NICE …` + `E AidlSensorManager:
...
(SCHED_OTHER) — was `10`/`1` (SCHED_FIFO). Every wandr-sensormanager thread is policy 0.
...
- **Non-reconnecting client cache:** `wandr-hal-sensors` cached the resolved
  `ISensorManager` + event queue on success and **never rebuilt** them, so after a
  `wandr-sensormanager` restart the arbiter/host latched a dead handle → sensors
...
**Fixed (2026-06-07) — cache half:** `wandr-hal-sensors` now `ping_binder`s the cached
...
`runtime/wandr-hal-sensors/src/lib.rs`, `runtime/wandr-arbiter/wandr-arbiter-bin/src/sensor_driver.rs`.
...
for the wandr-sensormanager-restart case**.

**Device-verified (2026-06-07):** killed `wandr-sensormanager` under a live arbiter →
...
client's BitTube to wandr-sensormanager hangs up while its server-side `EventQueue` is
still alive, the unguarded `handleEvent` busy-loops the poll thread at ~100% of one
core. Triggers: a `sensorservice` restart (wandr-sensormanager's *internal* BitTube to
...
**Reproduced live (2026-06-07).** A **duplicate** `wandr-sensormanager` was started
...
`wandr-sensormanager` at once; the `--no-art` bringup already `pkill`s old instances
...
duplicate exists, `kill -9` the orphan; otherwise restart `wandr-sensormanager` (drops
...
stable `wandr-sensormanager` + one stable arbiter event-queue connection, the poll
...
The wandr AIDL consumers and the camera's timing-sensitive EIS gyro share the one
...
- Single-client sensors HAL invariant holds (`lshal`: only sensorservice). `wandr-sensors`
  deleted — no second direct opener.
- HIDL (camera) and AIDL (wandr) are **separate** `android::SensorManager` instances in
  wandr-sensormanager (different package names) → independent ISensorServer connections.
- Java SensorManager being dead is fine — guests reach sensors through the `sensors` WIT
  (`wandr-host/src/sensors_impl.rs` → `wandr-hal-sensors` → AIDL ISensorManager).
...
thread `S`, nothing `R`, probe process already exited; wandr-sensormanager at 0%).
