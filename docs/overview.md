...
| **2048** (`repros/openswiftui-wasm`) | **Swift / OpenSwiftUI** | `wasi:canvas`† — render + AttributeGraph correctness proof; **moves are script-driven** (auto-move sequence), OpenSwiftUI's gesture/Event layer isn't wired to wasm input yet, so it isn't swipe-interactive | plays all moves, device-stable (spike) |
| **Swift canvas spike** (`repros/swift-canvas-spike`, `wandr.swift.canvas.test`) | **Swift** | `wasi:canvas`† · exports `wasi:input-handlers/pointer-handler`†+`frame-handler`† (real pointer/DragGesture input) · `wandr:ui-shell/frame-pacing` | device-verified render; 2048-on-real-gestures WIP |
